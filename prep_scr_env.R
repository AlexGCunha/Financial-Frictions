...
for (y in years){
  ...
  for (m in months){
    ...
    df <- read_dta(archive)
    df<-df %>% 
      select(c(time_id, loan_id, bank_id, firm_id,
...
for (y in years){
  ...
  df <- df %>% 
    ...
    mutate(firm_bank_start_date = ifelse(firm_start_date>firm_bank_start_date,NaN,firm_bank_start_date)) %>% 
    group_by(bank_id, firm_id) %>% 
    mutate(mean_start = mean(firm_bank_start_date)) %>% 
    ungroup() %>% 
