...
data_path = "C:/Users/xande/OneDrive/Documentos/Doutorado/RA/Financial_Frictions/Data/SCR"
...
for (y in years){
  for (m in months){
    ...
    df<- df %>% 
      ...
      mutate_at(c("loan_start_date"),~case_when(.<=as.Date("1jan1901",format='%d%b%Y')~as.Date(NA),TRUE~.)) %>% #input nan for dates befor 1901
      mutate(loan_start_year = format(loan_start_date, "%Y")) %>% 
      #filter only contracts that started in this year
      filter(loan_start_year==y)
    
    #sample 500 contracts per month
